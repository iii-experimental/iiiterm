...
export function renderPane(sessions: SessionState[]): string {
  const header =
  ...
  const rows = sessions
    ...
    .map((s) => {
      ...
      return `${status}   ${pad(agent, 6)} ${pad(title, 42)} ${pad(cwd, 28)} ${pad(ago(s.lastTurnAt), 5)} ${pad(tokens(s.tokensIn), 6)} ${pad(tokens(s.tokensOut), 6)}`;
    })
    .join('\n');

  return CLEAR + header + (rows || `${DIM}(no sessions yet — bridge workers still scanning)${RESET}`) + '\n';
}
